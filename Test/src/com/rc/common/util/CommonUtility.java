...
public class CommonUtility {
	...
	public static void createWebServiceURL(AdapterDTO adapterDTO) throws RCSystemException{
		...
		//System.out.println(eCommWSURL + " before param");
		
		//Add parameters
		logger.debug(METHOD_NAME+": add parameters");
		String paramURL = setParamInURL(adapterDTO);
		
		if (paramURL!=null){
			eCommWSURL+=paramURL;
		}
		
		System.out.println(eCommWSURL + " **** test URL after add param in Common Utility ****");
	...
	public static String setParamInURL(AdapterDTO adapterDTO){
		...
		if(requestParamMap == null || (apiParamURL != null && apiParamURL.contains("@"))){
			return apiParamURL;
		}
