...
public class WCSServiceImpl extends RCBaseService {
	...
	public ResponseDTO doService(Map<String, Object> aServiceMap) throws RCSystemException {
		...
		if(aServiceMap.containsKey(Constants.HEADER_PARAMETERS)) {
			
			@SuppressWarnings("unchecked")
			Map<String, String> requestHeaderParameters = (Map<String, String>)aServiceMap.get(Constants.HEADER_PARAMETERS);
			
			adapterDTO.setRequestHeaderParameters(requestHeaderParameters);			
		}
		
		adapterDTO.setBaseStoreConfigDTO(super.getApplicationData().getBaseStoreConfig());
		
		Adapter adapter = AdapterFactory.getAdapter(eCommPlatform); //finds the adapter corresponding to the E-Commerce platform called (11/28)
